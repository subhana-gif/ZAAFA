  ...
  >
    ...
    <>
      {(() => {
        ...
        return (
          <>
            <span className="line-through text-gray-500">AED {product.price}</span>
...
<div className="flex gap-6 overflow-x-auto pb-4 md:grid md:grid-cols-2 lg:grid-cols-4">
  {relatedProducts.map((p) => {
    ...
    const finalPrice = p.offer
      ? Math.max(
          Number(p.price) -
            (p.offer.discountType === "percentage"
              ? (Number(p.offer.discountValue) / 100) * Number(p.price)
              : Number(p.offer.discountValue)),
          0
        ).toFixed(0)
        ...
        <div className="p-4 text-center">
          ...
          {p.offer ? (
            <p className="text-sm mt-1">
              <span className="line-through text-gray-500">AED {p.price}</span>{" "}
              <span className="text-yellow-600 font-bold">AED {finalPrice}</span>
            </p>
          ) : (
